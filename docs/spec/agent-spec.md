...
class AgentManager {
  ...
  register(id: string, parentId: string | null, agentType: string, task: string, session: AgentSession) {
...
function createAgentTools(currentAgentId: string) {
  const delegateTool = defineTool({
    name: "delegate_task",
    description: "Delegate a sub-task to a specialized background agent. Use 'sync' to pause your own work and wait for the result. Use 'async' to fire-and-forget, allowing you to do other things until you are notified of completion.",
    ...
    execute: async (id, params) => {
      ...
      if (parentDepth >= 5) {
        return { content: [{ type: "text", text: "Error: Maximum delegation depth of 5 reached. Cannot delegate further." }] };
      }
      ...
      if (params.mode === "sync") {
        ...
        return { content: [{ type: "text", text: `Background task delegated to ${params.agentType}. You will be notified via followUp.` }] };
      }
    }
  });
...
async function runWorker(task: string, agentType: string, parentId: string) {
  ...
  if (contextPath) {
    ...
    console.warn(`Agent context file not found for type: ${agentType}. Checked local and global. Using default.`);
  }
...
async function runWorkerAsync(task: string, agentType: string, parentId: string) {
  ...
  await mainSession.followUp(`[SYSTEM]: Background agent (${agentType}) completed task. Result: ${result}`);
}
```
