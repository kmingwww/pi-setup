...
describe('createAgentTools', () => {
  ...
  it('delegate_task sync delegates to runWorker and returns result', async () => {
    ...
    const result = await delegateTool!.execute('some-call-id', {
  ...
  it('check_agent_statuses returns agent statuses from manager', async () => {
    ...
    const result = await statusTool!.execute('some-call-id', {} as any, undefined, undefined, {} as any);
  ...
  it('delegate_task rejects if depth >= 5', async () => {
    ...
    expect(result).toEqual({ content: [{ type: "text", text: "Error: Maximum delegation depth of 5 reached. Cannot delegate further." }], details: { agentType: 'researcher', tools: [] } });
  ...
  describe('async mode', () => {
    ...
    it('calls runWorkerAsync with correct params for async mode', async () => {
      ...
      await delegateTool!.execute('some-call-id', {
        agentType: 'researcher',
        task: 'find pi docs',
        mode: 'async'
      }, undefined, undefined, mockCtx);
    ...
    it('rejects async mode if depth >= 5', async () => {
      ...
      expect(result).toEqual({
        content: [{ type: "text", text: "Error: Maximum delegation depth of 5 reached. Cannot delegate further." }],
