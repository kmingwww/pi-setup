...
describe("notify extension", () => {
  ...
  describe("agent_end in TUI mode — Linux", () => {
    ...
    it("truncates long prompts to 72 chars", async () => {
      ...
      const longPrompt =
        "this is a very long user prompt that exceeds the maximum truncation length";
  ...
  describe("agent_end in TUI mode — Kitty", () => {
    it("writes OSC 99 escapes with context body", async () => {
      ...
      expect(stdoutWrite).toHaveBeenCalledWith(expect.stringContaining("\x1b]99;i=1:d=0;Pi\x1b\\"));
      expect(stdoutWrite).toHaveBeenCalledWith(
  ...
  describe("agent_end in non-TUI mode", () => {
    it.each(["print", "rpc", "json"] as const)("does nothing in %s mode", async (mode) => {
      const pi = createMockPi();
      const factory = await loadNotify({ notifySend: true });
      await factory(pi as any);
  ...
  describe("context extraction", () => {
    it("finds the last user message when other messages follow", async () => {
      ...
      expect(stdoutWrite).toHaveBeenCalledWith(expect.stringContaining(expectedBody("do X")));
    });
  ...
  describe("session_start — backends info", () => {
    it("notifies with desktop (Linux) and OSC backends", async () => {
      ...
      expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("OSC 777"), "info");
    });
    ...
    it("reports only OSC 777 when no other backends are found", async () => {
      ...
      expect(ctx.ui.notify).toHaveBeenCalledWith("Notify: OSC 777 (Background only)", "info");
    });
    ...
    it("reports Kitty backend when KITTY_WINDOW_ID is set", async () => {
      ...
      expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("OSC 99 (Kitty)"), "info");
    });
    ...
    it("reports Windows Toast backend when WT_SESSION + powershell are available", async () => {
      ...
      expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("Windows Toast"), "info");
    });
  });
