...
describe("buildContentSummary", () => {
  it("single selection", () =>
    expect(buildContentSummary({ question: "?", selections: [{ label: "A" }] })).toBe(
      "The user chose A.",
    ));
  it("multiple selections", () =>
    expect(
      buildContentSummary({ question: "?", selections: [{ label: "A" }, { label: "B" }] }),
  ...
  it("write-in only", () =>
    expect(buildContentSummary({ question: "?", selections: [], answer: "custom" })).toBe(
      'The user wrote: "custom".',
    ));
  it("empty", () =>
    expect(buildContentSummary({ question: "?", selections: [] })).toBe(
...
describe("execute — empty options", () => {
  it("returns error", async () => {
    ...
    const result = await tool.execute("id", { question: "?", options: [] }, undefined, undefined, {
      mode: "tui",
      ui: { custom: vi.fn() },
    });
